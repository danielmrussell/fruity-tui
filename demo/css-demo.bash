...
_page_annotations() {
    ...
    2)  _ann spec "#spec is an editable ft-textfield. The .class you give it decides which rule below paints it."
        _ann selClass "Choose a class. .warning → crimson + bold; .ok → green. The bash pane shows the exact ft-modify call." ;;
...
# real CSS and drives it live by manipulating the ELEMENT (ft-modify class / property / --custom-
...
_demo_css() {
    ...
    5)  DCSS='#spec { color: var(--accent); }' ;;   # STATIC; the dropdown re-points --accent ON #spec
                                                     # itself (ft-modify spec --accent=…), like the DOM's
    ...
    7)  # All five pseudo-element rules are STATIC and class-GATED (#spec.pe-caret::caret …). The
        # dropdown just sets #spec's class (ft-modify spec class=pe-…), so exactly one is ever in
    ...
    8)  # The @keyframes are DEFINED in the sheet (they are fancy CSS — a stylesheet thing).
        # But PLAYING one is just a property: #spec's `animation` is set live with ft-modify,
...
#   · changing a PROPERTY (every other page): ft-modify dirties by kind, subtree-wide when
...
_refresh_code_panes() {
    ...
    _page_content
    ft-modify css  value="$CSS"
    ft-modify bash value="$BASH"
    return 0
}
...
# Setting the property is the whole of it: color and font-weight INHERIT, so ft-modify
# repaints the subtree that inherits them. (These three used to carry a ft_dirty_subtree each.)
inhColor_on_change()    { INH_COLOR=$1; ft-modify inhbox color="$1";        return 0; }
inhBold_on_activate()   { INH_BOLD=on;  ft-modify inhbox fontWeight=bold;   return 0; }
inhBold_on_deactivate() { INH_BOLD=off; ft-modify inhbox fontWeight=normal; return 0; }

# 2 Selectors — a class on the specimen decides which rule matches it.
selClass_on_change() { SEL_CLASS=$1; ft-modify spec class="${1/none/}"; return 0; }
...
stDisabled_on_activate()   { ST_DISABLED=on;  ft-modify spec disabled=true;  return 0; }
stDisabled_on_deactivate() { ST_DISABLED=off; ft-modify spec disabled=false; return 0; }
...
varPick_on_change() { VAR_ACCENT=$1; ft-modify spec --accent="$1"; _refresh_code_panes; return 0; }
...
cfFmt_on_change() { CF_FMT=$1; ft-modify spec color="$1"; return 0; }
...
_setup_struct_state() {
    ...
    [[ "$STRUCT" == placeholder ]] && ft-modify spec value="" || ft-modify spec value="$SPEC"
    case "$STRUCT" in
        caret)     ft-modify spec runlevel=editing; FT_TEXTFIELD_CARET[spec]=6 ;;
        selection) ft-modify spec runlevel=editing; FT_TEXTFIELD_MARK[spec]=4; FT_TEXTFIELD_CARET[spec]=19 ;;
        *)         ft-modify spec runlevel=unfocused ;;
    esac
    return 0
}
structPick_on_change() { STRUCT=$1; ft-modify spec class="pe-$STRUCT"; _setup_struct_state; _refresh_code_panes; return 0; }
...
# with ft-modify (name + duration), exactly what the bash code pane shows.
animPick_on_change() { AN_NAME=$1; ft-modify spec animation="$AN_NAME ${AN_DUR}s"; _refresh_code_panes; return 0; }
animDur_on_change()  { AN_DUR=$1;  ft-modify spec animation="$AN_NAME ${AN_DUR}s"; _refresh_code_panes; return 0; }
...
cbNest_on_activate()   { CB_NEST=on;  ft-modify combox class=spotlight; return 0; }
cbNest_on_deactivate() { CB_NEST=off; ft-modify combox class="";   return 0; }
...
_goto_step() {
    ...
    _place_callout
    # NOTHING PAINTS HERE. ft_remove damaged what the callout covered and every ft-modify above
...
_page_content() {
    ...
    1)  title="1 · Inheritance"
        ...
        # code panes show the RELEVANT rule + the property (ft-modify) that changes it live.
...
ft-modify inhbox color=dodgerblue     #  ▸ the whole subtree re-tints' ;;
...
ft-modify spec class=warning         #  ▸ .warning matches
ft-modify spec class=ok              #  ▸ .ok matches
ft-modify spec class=""              #  ▸ no class → default look' ;;
...
ft-modify spec disabled=true         #  ▸ :disabled now matches
...
ft-modify spec --accent=dodgerblue     #  ▸ el.style.setProperty("--accent", …)
...
ft-modify spec color=crimson
ft-modify spec color=#dc143c
ft-modify spec color="rgb(220,20,60)"
ft-modify spec color="hsl(348,83%,47%)"    #  ▸ all the same colour' ;;
    7)  title="7 · Pseudo-elements (substructures)"
        ...
        case "$STRUCT" in
            ...
            caret)       _rule='#spec.pe-caret::caret { background: magenta; color: 16; }'
                         _setup='ft-modify spec runlevel=editing   # enter edit mode so the ::caret is drawn' ;;
...
ft-modify spec value=""            #  ▸ empty the field so the ::placeholder shows' ;;
...
ft-modify spec class=pe-$STRUCT
...
ft-modify spec animation=\"$AN_NAME ${AN_DUR}s\"
...
/* matches a textfield ONLY inside a .spotlight ancestor */'
    ...
    ft-textfield name=spec …          #   INSIDE  → can match
end_ft_div
ft-textfield name=specOut …           # OUTSIDE → never matches (the proof)
# ── the checkbox toggles the wrapper'"'"'s class, live: ──
ft-modify combox class=spotlight      #  ▸ only #spec turns gold' ;;
    10) title="10 · Themes"
...
_show_page() {
    ...
    ft-empty stage
        ft-frame name=win title="The Fruity CSS engine — $title  ($PAGE/$LAST)" \
            ...
            ft-label name=concept width=88 color=subtext "$CONCEPT"
            ...
            1)  ft-div name=inhbox color="$INH_COLOR" display=flex flexDirection=column gap=0 alignItems=center
                    ...
                    ft-label name=inhNote "↑ this label inherits the same crimson"
                end_ft_div
                ft-div name=ctl1 display=flex gap=4 alignItems=center
                    ft-div name=grp381 display=flex gap=1 alignItems=center
                        ft-label "Parent colour:"
                        ft-select name=inhColor size=1 onChange=inhColor_on_change
                            ft-option value=crimson    "Crimson"
                            ft-option value=dodgerblue "Blue"
                            ft-option value=46         "Green"
                            ft-option value=gold       "Gold"
                        end_ft_select
                    end_ft_div
                    ...
                    ft-checkbox name=inhBold "Bold" accessKey=O onActivate=inhBold_on_activate onDeactivate=inhBold_on_deactivate
                end_ft_div ;;
            2)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true class="${SEL_CLASS/none/}"
                ft-div name=ctl2 display=flex gap=1 alignItems=center
                    ...
                    ft-select name=selClass size=1 onChange=selClass_on_change
                        ft-option value=none    "(none)"
                        ft-option value=warning ".warning"
                        ft-option value=ok      ".ok"
                    end_ft_select
                end_ft_div ;;
            3)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true class=hot
                ft-div name=ctl3 display=flex gap=2 alignItems=center
                    ft-label "Rules on:"
                    ft-checkbox name=spType  "textfield" accessKey=T checked=true onActivate=spType_on_activate onDeactivate=spType_on_deactivate
                    ft-checkbox name=spClass ".hot"      accessKey=C onActivate=spClass_on_activate onDeactivate=spClass_on_deactivate
                    ft-checkbox name=spId    "#spec"     accessKey=I onActivate=spId_on_activate onDeactivate=spId_on_deactivate
                end_ft_div ;;
            4)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true
                ft-div name=ctl4 display=flex gap=2 alignItems=center
                    ft-checkbox name=stDisabled "Disable the box" accessKey=D onActivate=stDisabled_on_activate onDeactivate=stDisabled_on_deactivate
            ...
            5)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true --accent="$VAR_ACCENT"
                ft-div name=ctl5 display=flex gap=1 alignItems=center
                    ...
                    ft-select name=varPick size=1 onChange=varPick_on_change
                        ft-option value=crimson    "crimson"
                        ft-option value=dodgerblue "dodgerblue"
                        ft-option value=46         "green"
                        ft-option value=201        "magenta"
                    end_ft_select
                end_ft_div ;;
            6)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true color="$CF_FMT"
                ft-div name=ctl6 display=flex gap=1 alignItems=center
                    ...
                    ft-select name=cfFmt size=1 onChange=cfFmt_on_change
                        ft-option value=crimson            "crimson (name)"
                        ft-option value=#dc143c            "#dc143c (hex)"
                        ft-option value="rgb(220,20,60)"   "rgb(220,20,60)"
                        ft-option value="hsl(348,83%,47%)" "hsl(348,83%,47%)"
                    end_ft_select
                end_ft_div ;;
            7)  ft-textfield name=spec value="$SPEC" size=44 rows=2 wrap=true class="pe-$STRUCT" placeholder="#spec is empty — so you see its ::placeholder"
                ft-div name=ctl7 display=flex gap=1 alignItems=center
                    ft-label "Pseudo-element:"
                    ft-select name=structPick size=1 onChange=structPick_on_change
                        ft-option value=border      "::border"
                        ft-option value=selection   "::selection"
                        ft-option value=scrollbar   "::scrollbar"
                        ft-option value=caret       "::caret"
                        ft-option value=placeholder "::placeholder"
                    end_ft_select
                end_ft_div ;;
                ...
                ft-div name=ctl8 display=flex gap=4 alignItems=center
                    ft-div name=grp446 display=flex gap=1 alignItems=center
                        ...
                        ft-select name=animPick size=1 onChange=animPick_on_change
                            ft-option value=none  "none"
                            ft-option value=glow  "glow"
                            ft-option value=alarm "alarm"
                        end_ft_select
                    end_ft_div
            ...
            9)  ft-div name=combox display=flex flexDirection=column gap=0 alignItems=center
                ...
                ft-div name=ctl9 display=flex gap=1 alignItems=center
                    ft-checkbox name=cbNest "Wrap the first box in a .spotlight" accessKey=C onActivate=cbNest_on_activate onDeactivate=cbNest_on_deactivate
                end_ft_div ;;
    ...
    (( PAGE == 1 ))       && ft-modify btnBack     disabled=true    # first page → no previous page
    (( STEP == 1 ))       && ft-modify btnStepPrev disabled=true    # first step → no previous step
    (( STEP == nsteps ))  && ft-modify btnStepNext disabled=true    # last step → nothing more to step to
    [[ "$PAGE" == 4 && "$ST_DISABLED" == on ]] && ft-modify spec disabled=true
    ...
    ft-modify spec wrapIndicator="$SHOW_WRAP" newlineIndicator="$SHOW_NL"
    ...
    ft-modify navbar status="Page $PAGE of $LAST — $title    ·    ◀ ▶ step through · Okay / Back = page · Q quit"
...
# Live toggles for #spec's line indicators (shown off on every page). ft-modify
...
_toggle_wrap() { [[ "$SHOW_WRAP" == true ]] && SHOW_WRAP=false || SHOW_WRAP=true; ft-modify spec wrapIndicator="$SHOW_WRAP"; }
_toggle_nl()   { [[ "$SHOW_NL"   == true ]] && SHOW_NL=false   || SHOW_NL=true;   ft-modify spec newlineIndicator="$SHOW_NL"; }
