...
note "ft-modify accepts a spaced multi-class value now that class is registered"
ft-modify lbl class="c d e"
ft_get lbl class; check "multi-class via ft-modify" "$FT_RET" "c d e"
...
ft-label name=tmp parent=box "temp"
...
h2() { LOG+="h2($1) "; }
h3() { LOG+="h3 "; }
ft-button name=evb parent=box onActivate=h1 onActivate=h2 "Ev"
...
LOG=""; ft-modify evb onActivate=h1
...
ft-button name=cvb parent=box "C"                # note: NOT wired
...
ft-label name=sty parent=box style="color: 201; font-weight: bold" "S"
...
ft-modify sty style="--tint: 46"
ft_style sty --tint;     check "custom property via style= (--tint)" "$FT_RET" "46"

note "ft_remove_attribute: truly unsets — falls back to the stylesheet/class default"
ft_stylesheet name=rat style='#raEl { color: 33; }'
ft-label name=raEl parent=box color=99 "R"
ft_style raEl color; check "inline color wins first" "$FT_RET" "99"
ft_remove_attribute raEl color
...
ft-div name=corig class=spotty --tone=7
  ft-label name=ckid color=45 "kid"
end_ft_div
...
ft-label name=nx parent=tbox "x"                        # create appends at end
...
ft-label name=nr parent=tbox "r"                        # tbox: n1 n2 n3 nx nr
