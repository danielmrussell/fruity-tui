#!/usr/bin/env bash
# Unit tests for the class system: constructor-chain inheritance (a subclass
# class-constructor calls its superclass's, then overrides struct entries),
# memoized one-time class registration, per-class property defaults, and the
# nesting DSL (auto-parent from the open-container stack, end_ft_* type
# checking, <type>_on_children_complete hooks).
...
check "button inherited label's height fn"          "${FT_CLASS_HEIGHT[button]}" "_ft_height_label"
check "button OVERRODE the width fn (adds its own padding)" "${FT_CLASS_PREFERRED_WIDTH[button]}" "_ft_preferred_width_button"
check "button OVERRODE the draw fn"                 "${FT_CLASS_DRAW[button]}"  "_ft_draw_button"
check "label keeps its own draw fn"                 "${FT_CLASS_DRAW[label]:-unset}" "unset"
ft_class_init label
check "label's own struct untouched by button's"    "${FT_CLASS_DRAW[label]}"   "_ft_draw_label"
...
ft_class_biggadget() {
    ft_class extends=gadget defaults="padding=2"   # superclass first, then extend
}
...
note "draw/preferredWidth/height bind themselves from _ft_<role>_<class>, and INHERIT"
# The convention runs for each class in the chain as its constructor fires, not just for the
# leaf — otherwise a subclass with no function of its own inherits nothing. That exact slip
# gave every button height 0, which made it invisible to the hit test: no click could land.
_ft_draw_widget()            { :; }
_ft_height_widget()          { FT_RET=1; }
_ft_preferred_width_widget() { FT_RET=1; }
ft_class_widget() { ft_class extends=ft_control focusable=true; }
ft-widget() { ft_new widget "$@"; }
_ft_draw_gizmo() { :; }                      # overrides draw ONLY
ft_class_gizmo() { ft_class extends=widget; }
ft-gizmo() { ft_new gizmo "$@"; }
ft-widget name=cw1; ft-gizmo name=cg1
check "the class binds its own draw"      "${FT_CLASS_DRAW[widget]}"             _ft_draw_widget
check "…and height"                       "${FT_CLASS_HEIGHT[widget]}"           _ft_height_widget
check "…and preferredWidth"               "${FT_CLASS_PREFERRED_WIDTH[widget]}"  _ft_preferred_width_widget
check "a subclass overrides what it has"  "${FT_CLASS_DRAW[gizmo]}"              _ft_draw_gizmo
check "…and INHERITS what it does not"    "${FT_CLASS_HEIGHT[gizmo]}"            _ft_height_widget
check "…for every conventional key"       "${FT_CLASS_PREFERRED_WIDTH[gizmo]}"   _ft_preferred_width_widget
check "the real chain: button gets a height fn" "${FT_CLASS_HEIGHT[button]}"     _ft_height_label

note "a short function value expands by convention; 'none' clears an inherited one"
check "keymap=activate  → ft_keymap_activate"  "${FT_CLASS_KEYMAP[button]}" ft_keymap_activate
check "mouse=activate   → _ft_mouse_activate"  "${FT_CLASS_MOUSE[button]}"  _ft_mouse_activate
check "focusSkip=none clears label's skip fn"  "${FT_CLASS_FOCUS_SKIP[button]}" ""
check "…which label itself still has"          "${FT_CLASS_FOCUS_SKIP[label]}"  _ft_label_focus_skip

note "ft_class REFUSES what the raw array pokes accepted silently"
# The whole reason the authoring API exists: FT_CLASS_FOCUSSABLE[$c]=1 was a valid bash
# assignment that created a new array nobody reads. Each of these must fail LOUDLY.
ft_class_typo() { ft_class extends=ft_control focussable=true; }
ft-typo() { ft_new typo "$@"; }
...
ft_class_notbool() { ft_class extends=ft_control focusable=yes; }
...
ft_class_nosuper() { ft_class extends=nosuchclass; }
ft-nosuper() { ft_new nosuper "$@"; }
err=$(ft-nosuper name=t3 2>&1)
check "an unknown superclass is rejected" "$(case "$err" in *'unknown class'*nosuchclass*) echo yes ;; *) echo "$err" ;; esac)" yes

# extends= runs the SUPERCLASS constructor, so arriving late it would overwrite keys already
# set — silently turning a focusable subclass back into its parent's unfocusable self.
ft_class_lateextend() { ft_class draw=_ft_draw_label; ft_class extends=ft_control; }
...
# clicks. Every class must therefore CARRY the flag, not merely omit it when false.
ft-heading name=hbool "x"
ft-button  name=bbool "y"
check "focusable=false stored as 0"      "${FT_CLASS_FOCUSABLE[heading]}" 0
check "focusable=true stored as 1"       "${FT_CLASS_FOCUSABLE[button]}"  1
check "an undeclared boolean is 0 too"   "${FT_CLASS_NOHIT[heading]}"     0
check "…and fillsBackground likewise"    "${FT_CLASS_FILLS_BACKGROUND[heading]}" 0
check "a class that declares it gets 1"  "${FT_CLASS_FILLS_BACKGROUND[form]}"    1
...
# ft_class_init calls "ft_class_<type>", and `extends=` calls the SUPERCLASS constructor
# directly — so a class that extends itself, or a ring of them, recursed until bash's stack
# blew and the process died of a SEGFAULT: no message, no trap, nothing for the app to report.
# FT_CLASS_READY cannot catch it, being set only on the way out. Each case must now return.
...
check "a class that extends ITSELF"      "$(_cyc 'ft_class_selfy() { ft_class extends=selfy; }; ft_class_init selfy')" "returned"
check "a ring: aa extends bb extends aa" "$(_cyc 'ft_class_aa() { ft_class extends=bb; }; ft_class_bb() { ft_class extends=aa; }; ft_class_init aa')" "returned"
# A type whose NAME collides with a framework function: "ft_class_init" IS the initialiser, so
# `ft_class_init init` used to call itself. (Found by a probe that enumerated class names by
# grepping ^ft_class_* and matched the initialiser.)
check "a type named after the initialiser" "$(_cyc 'ft_class_init init')" "returned"
check "an EMPTY type name"                 "$(_cyc 'ft_class_init ""')"  "returned"
# …and a control of a cyclic class is simply never built, rather than half-built.
check "a control of a cyclic class is not created" \
      "$(_cyc 'ft_class_selfy() { ft_class extends=selfy; }
...
check "button still inherits a height"  "$(_cyc 'ft_class_init button; [[ -n "${FT_CLASS_HEIGHT[button]:-}" ]] || exit 3')" "returned"
check "…and its own draw"               "$(_cyc 'ft_class_init button; [[ "${FT_CLASS_DRAW[button]:-}" == _ft_draw_button ]] || exit 3')" "returned"


# ═══ A CLASS DEFAULT IS CASCADE LEVEL 5 ══════════════════════════════════════
...
# that 26 classes default were unreachable from a stylesheet, on every control.
...
# Asked of a REAL class: ft_control is only ever initialised as somebody's ancestor, and the
# chain is appended into the SUBCLASS's entry. A frame declares no importance of its own, so
# finding one there is finding the base control's.
check "the base control declares importance" \
      "$(case " ${FT_CLASS_DEFAULTS[frame]} " in *" importance=normal "*) echo yes ;; *) echo no ;; esac)" "yes"
...
# class default is a declaration for this element. Two of the 79 defaults inherit: a tree's
...
# The resolver treats an empty class default as "declares nothing" — one assoc read instead of a
# presence test and a second read, worth 40ms on a 37-control layout. That is only sound while no
# class default is BOTH inherited AND empty. This is the assertion that catches the day one is.
_t_bad=""
for _fn in $(declare -F | sed -n 's/^declare -f ft_class_//p'); do ft_class_init "$_fn" >/dev/null 2>&1; done
for _k in "${!FT_CLASS_DEFAULT[@]}"; do
    [[ -n "${FT_CLASS_DEFAULT[$_k]}" ]] && continue
...
check "…and the table is not empty (the check has teeth)" \
      "$(( ${#FT_CLASS_DEFAULT[@]} > 100 ))" "1"

note "a class question asked with NO control writes nothing to stderr"
# ${FT_TYPE[""]} is a bash error, and in a TUI stderr is the alt screen. Probes reach the class
# helpers without an instance (a height function called for its class-level answer), so each one
# guards the empty name. run-all fails a file that writes to stderr; this fails the assertion.
_t_err=$( { _ft_class_default "" display; _ft_class_declares "" display
            _ft_prop_or_class "" display ""; _ft_disp ""; _ft_border ""; } 2>&1 >/dev/null )
...
# field of every registered class against the table it was built from.
#
# The ninth field is not a property DEFAULT but a class SLOT — topEdgeProp, the name of the
# property this class draws on its top edge — so it is checked against its own table.
...
for _fn in $(declare -F | sed -n 's/^declare -f ft_class_//p'); do ft_class_init "$_fn" >/dev/null 2>&1; done
for _ty in "${!FT_CLASS_READY[@]}"; do
    set -- ${FT_CLASS_INSET_BOX[$_ty]-}
    (( $# == 9 )) || { _t_bad+=" $_ty:inset-has-$#-fields"; continue; }
    _i=0
    for _p in "${_t_inset_props[@]}"; do
        _i=$(( _i + 1 )); _want=${FT_CLASS_DEFAULT["$_ty $_p"]-}
        if [[ -z "$_want" ]]; then
    ...
    _want=${FT_CLASS_TOP_EDGE_PROP[$_ty]:--}
    [[ -n "${FT_CLASS_TOP_EDGE_PROP[$_ty]:-}" ]] && (( _t_nonfallback++ ))
    _t_fields=$(( _t_fields + 1 ))
    [[ "$9" == "$_want" ]] || _t_bad+=" $_ty.topEdgeProp($9≠$_want)"
    set -- ${FT_CLASS_MARGIN_BOX[$_ty]-}
    (( $# == 5 )) || { _t_bad+=" $_ty:margin-has-$#-fields"; continue; }
    _i=0
    for _p in "${_t_margin_props[@]}"; do
        _i=$(( _i + 1 )); _want=${FT_CLASS_DEFAULT["$_ty $_p"]-}; [[ -z "$_want" ]] && _want=0
        _t_fields=$(( _t_fields + 1 ))
        [[ "${!_i}" == "$_want" ]] || _t_bad+=" $_ty.$_p(${!_i}≠$_want)"
    done
done
...
# least some fields must carry a value the class actually declared.
...
check "tabs keeps the three rows it reserves for its strip" \
      "$(set -- ${FT_CLASS_INSET_BOX[tabs]-}; echo "$3")" "3"
check "a frame still defaults to having a border" \
      "$(set -- ${FT_CLASS_INSET_BOX[frame]-}; echo "$1")" "true"
