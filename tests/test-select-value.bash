...
#  nowhere else. So `ft_set se selectedIndex=2` PAINTED the third option while `ft_get se
#  value` still answered the first one's: the control told the app one thing and the user
#  another, at the same time. The reverse direction did not exist at all — `ft_set se
...
ft-form name=app width=70 height=24
    ft-select name=bare width=20                     # the short form: no value= anywhere
        ft-option text="alpha"; ft-option text="bravo"; ft-option text="charlie"
    end_ft_select
    ft-select name=full width=20                     # values spelled out
        ft-option value=a text="Alpha"; ft-option value=b text="Bravo"; ft-option value=c text="Charlie"
    end_ft_select
    ft-select name=multi width=20 multiple=true
        ft-option value=x text="Ex" selected=true; ft-option value=y text="Why"; ft-option value=z text="Zed" selected=true
    end_ft_select
end_ft_form
ft_layout app
FT_ROOT=app
...
ft-form name=app2 width=70 height=24
    ft-select name=ph width=24
        ft-option value="" text="Choose…"; ft-option value=r text="Red"
    end_ft_select
end_ft_form
ft_layout app2
...
ft-form name=app3 width=70 height=24
    ft-select name=dup width=20
        ft-option value=same text="First"; ft-option value=other text="Middle"; ft-option value=same text="Last"
    end_ft_select
end_ft_form
ft_layout app3
ft_set dup selectedIndex=2
check "the third option is selected" "$(_idx dup)" "2"
check "…and its value is 'same'"     "$(_val dup)" "same"
ft_set dup value=same                 # exactly what it already answers
check "…and re-writing that value stays put" "$(_idx dup)" "2"
ft_set dup value=other
...
ft-form name=app4 width=70 height=24
    ft-multitoggle name=mt text="Mode"
        ft-option text="on" glyph="[x]"; ft-option text="off" glyph="[ ]"
    end_ft_multitoggle
end_ft_form
ft_layout app4
